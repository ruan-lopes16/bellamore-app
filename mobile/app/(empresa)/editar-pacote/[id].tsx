...
export default function EditarPacote() {
  ...
  useEffect(() => {
    ...
    supabase
      .from('pacotes')
      .select(`*, pacote_servicos(quantidade, servico:servicos(*))`)
      .eq('id', id)
      .single()
      .then(({ data }) => {
        if (data) {
          ...
          setValidade(data.validade_dias ?? 90);
          const sel: Record<string, number> = {};
          (data.pacote_servicos as any[]).forEach((ps) => {
            sel[ps.servico.id] = ps.quantidade;
          });
          setSelecionados(sel);
        }
        setCarregando(false);
  ...
  async function salvar() {
    ...
    const { error: errPacote } = await supabase
      .from('pacotes')
      .update({
        nome:          nome.trim(),
    ...
    if (errPacote) {
      setSalvando(false);
      Alert.alert('Erro', errPacote.message);
      return;
    }
  ...
  function desativar() {
    Alert.alert(
      'Desativar pacote',
      `"${nome}" não aparecerá para novos clientes, mas o histórico fica intacto.`,
      [
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }} keyboardShouldPersistTaps="handled">
        ...
        <LinearGradient colors={['#2C1654', '#3D1F72']} style={{ paddingTop: insets.top + 12, paddingHorizontal: 24, paddingBottom: 24 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <TouchableOpacity
              onPress={() => router.back()}
              style={{ width: 34, height: 34, backgroundColor: 'rgba(255,255,255,0.1)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)', borderRadius: 10, alignItems: 'center', justifyContent: 'center' }}
            >
