 ...
 * header + seletor mês + KPIs (grid auto-fit, 6-8 cards) + Despesas (topo,
 * largura total) + Evolução (12 meses) + Top Serviços + grid 2 colunas
 */
export default function FinanceiroLoading() {
  return (
    <div>
      ...
      {/* KPIs — grid auto-fit, sem buraco na última linha */}
      <div className="grid gap-3 sm:gap-4 mb-6 grid-cols-[repeat(auto-fit,minmax(150px,1fr))]">
        {[1,2,3,4,5,6].map(i => (
          <div key={i} className="bg-surface border border-border rounded-2xl p-3 sm:p-5 shadow-sm">
            ...
            <Sk className="h-3 w-3/4 max-w-[120px]" />
          </div>
        ))}
      </div>

      {/* Despesas — sempre no topo, largura total */}
      ...
      {/* Grid de 2 colunas: Evolução (12 meses) + Top Serviços */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        {/* Evolução mensal — 12 barras, rolagem horizontal */}
        <div className="bg-surface border border-border rounded-2xl p-5 shadow-sm">
          ...
          <div className="flex items-end gap-3 h-32 overflow-hidden">
      ...
      {/* Formas de pagamento — largura total */}
      <div className="bg-surface border border-border rounded-2xl overflow-hidden shadow-sm">
        <Sk className="h-5 w-1/3 max-w-[160px] mx-5 mt-5 mb-4" />
        {[1,2,3].map(i => (
          <div key={i} className="flex items-center gap-3 px-5 py-3 border-b border-border last:border-0">
            <Sk className="w-8 h-8 rounded-lg flex-shrink-0" />
